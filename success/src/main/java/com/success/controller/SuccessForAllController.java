...
public class SuccessForAllController {
	...
	public String searchConcerns(@RequestParam(value="searchTerm", required=false, defaultValue="") String searchTerm, Model model) {
	    String enhancedTerm = searchTerm + "";
	    model.addAttribute("userDTO", new UserDTO());
	    try {
			List<ConcernsDTO> fetchConcerns = successForAllService.fetchConcerns(searchTerm);
		} catch (Exception e) {
